...
export const progresoService = {
    ...
    /**
     * @method getById
     * @description Simula la obtención de una publicación de progreso por su ID.
     * @param {number} id - El ID de la publicación a obtener.
     * @returns {Promise<PublicacionProgresoDTO | undefined>} Una promesa que resuelve con la publicación o undefined si no se encuentra.
     */
    getById: async (id: number): Promise<PublicacionProgresoDTO | undefined> => {
        return new Promise((resolve) => {
            setTimeout(() => {
                const post = mockProgresoPosts.find(p => p.id === id);
                resolve(post);
            }, 200);
        });
    },
